...
import { DEFAULT_CACHE_MAX_KEYS, DEFAULT_CACHE_TTLS, DEFAULT_SESSION_CLEANUP_CONFIG, KEY_BUNDLE_TYPE, MIN_PREKEY_COUNT, PLACEHOLDER_MAX_AGE_SECONDS, STATUS_EXPIRY_SECONDS } from '../Defaults/index.js';
import { ReachoutTimelockEnforcementType, WAMessageStatus, WAMessageStubType } from '../Types/index.js';
import { ACCOUNT_RESTRICTED_TEXT, aesDecryptCTR, aesEncryptGCM, cleanMessage, cleanupCorruptedSession, compactError, Curve, decodeMediaRetryNode, decodeMessageNode, decryptMessageNode, delay, derivePairingCodeKey, encodeBigEndian, encodeSignedDeviceIdentity, extractAddressingContext, extractE2ESessionFromRetryReceipt, getCallStatusFromNode, getDecryptionJid, getHistoryMsg, getNextPreKeys, getStatusFromReceiptType, handleIdentityChange, hkdf, isCorruptedSessionError, makeMsmsgSecretCache, MISSING_KEYS_ERROR_TEXT, NACK_REASONS, NO_MESSAGE_FOUND_ERROR_TEXT, normalizeKeyLidToPn, normalizeMessageJids, resolveContactPictureIdentity, resolveLidToPn, safeCacheSet, SERVER_ERROR_CODES, toNumber, unixTimestampSeconds, xmppPreKey, xmppSignedPreKey } from '../Utils/index.js';
import { logMessageReceived, logTcToken } from '../Utils/baileys-logger.js';
...
import { metrics, recordHistorySyncMessages, recordMessageFailure, recordMessageReceived, recordMessageRetry } from '../Utils/prometheus-metrics.js';
import { buildAckStanza } from '../Utils/stanza-ack.js';
import { isRegularUser, isTcTokenExpired, resolveTcTokenJid, storeTcTokensFromIqResult } from '../Utils/tc-token-utils.js';
...
const isValidEnforcementType = (value) => value !== undefined && ENFORCEMENT_TYPE_VALUES.has(value);
export const makeMessagesRecvSocket = (config) => {
    const { logger, retryRequestDelayMs, maxMsgRetryCount, getMessage, shouldIgnoreJid, enableAutoSessionRecreation, enableCTWARecovery, sessionCleanupConfig } = config;
    ...
    const { userDevicesCache, devicesMutex, ev, authState, ws, messageMutex, notificationMutex, receiptMutex, signalRepository, sessionActivityTracker, query, upsertMessage, resyncAppState, onUnexpectedError, assertSessions, sendNode, relayMessage, sendReceipt, uploadPreKeys, sendPeerDataOperationMessage, messageRetryManager, getPrivacyTokens, registerSocketEndHandler, 
    // Port de upstream `4dbbba2891` (PR #2442)
    fetchAccountReachoutTimelock } = sock;
    ...
    const identityInFlightRefreshes = new Set();
    ...
    const retryRequestActiveJids = new Set();
    let tcTokenIndexSaveTimer;
     ...
     */
    const pdoCleanupTimers = new Set();
    ...
    const pruneExpiredTcTokens = async () => {
        ...
        if (pruneCount > 0) {
            ...
            for (const jid of survivingJids)
                tcTokenKnownJids.add(jid);
    ...
    const requestPlaceholderResend = async (messageKey, msgData) => {
        ...
        const alreadyHandled = await placeholderResendLocks.withLock({ namespace: '__placeholder_resend__', id: resendId }, async () => {
            const alreadyRequested = await placeholderResendCache.get(resendId);
            if (alreadyRequested) {
            ...
            try {
                ...
                logger.debug({ resendId }, 'placeholderResendCache full, marker skipped (will still send PDO unconditionally)');
            }
            return false;
        });
        if (alreadyHandled)
        ...
        if (markerWritten && !(await placeholderResendCache.get(resendId))) {
        ...
        if (msgData && stanzaId) {
            ...
            logger.debug({ messageKey: messageKey.id, stanzaId }, 'CTWA: Cached metadata using stanzaId for PDO response lookup');
        }
        ...
        const cleanupTimer = setTimeout(async () => {
            ...
            if (await placeholderResendCache.get(stanzaId)) {
    ...
    const handleReachoutTimelockNotification = (data) => {
        ...
        const timeEnforcementEnds = Number.isFinite(tsParsed) && tsParsed > 0 ? new Date(tsParsed * 1000) : new Date(Date.now() + 60000);
    ...
    const handleMexNewsletterNotification = async (node) => {
        ...
        try {
            ...
            const jsonText = typeof payloadContent === 'string' ? payloadContent : Buffer.from(payloadContent).toString('utf8');
    ...
    const handleMexNotification = async (node) => {
        ...
        if (updateNode && opName === 'NotificationUserReachoutTimelockUpdate') {
            ...
            if (!updateNode.content) {
                ...
                try {
                    const parsed = JSON.parse(updateNode.content.toString());
                    if (parsed?.data)
                        handleReachoutTimelockNotification(parsed.data);
                }
                catch (err) {
        ...
        if (updateNode && opName === 'MessageCappingInfoNotification') {
            ...
            if (!updateNode.content) {
                ...
                try {
                    const parsed = JSON.parse(updateNode.content.toString());
                    if (parsed?.data)
                        handleMessageCappingNotification(parsed.data);
                }
                catch (err) {
     ...
     */
    const offerCall = async (jid, isVideo) => {
        const meId = authState.creds.me?.id;
        if (!meId)
            throw new Boom('Not authenticated', { statusCode: 401 });
        ...
        offerContent.push({ tag: 'net', attrs: { medium: '3' }, content: undefined }, { tag: 'capability', attrs: { ver: '1' }, content: undefined }, { tag: 'enc', attrs: { v: '2', type: isVideo ? 'msg' : 'pkmsg' }, content: undefined }, { tag: 'encopt', attrs: { keygen: '2' }, content: undefined });
    ...
    const acceptCall = async (callId, callFrom, isVideo) => {
        ...
        acceptContent.push({ tag: 'net', attrs: { medium: '2' }, content: undefined }, { tag: 'encopt', attrs: { keygen: '2' }, content: undefined });
        const stanza = {
            tag: 'call',
            attrs: {
                from: meId,
    ...
    const preacceptCall = async (callId, callCreator, isVideo) => {
        const preacceptContent = [{ tag: 'audio', attrs: { rate: '16000', enc: 'opus' }, content: undefined }];
        if (isVideo) {
        ...
        preacceptContent.push({ tag: 'encopt', attrs: { keygen: '2' }, content: undefined }, { tag: 'capability', attrs: { ver: '1' }, content: undefined });
        const stanza = {
            tag: 'call',
            attrs: {
                to: callCreator,
    ...
    const sendTransport = async (callId, callCreator, to, candidates, round) => {
        ...
        const teChildren = candidates.map(c => ({
            tag: 'te',
     ...
     */
    const createCallLink = async (media = 'video', event, timeoutMs) => {
        const stanza = {
            tag: 'call',
            attrs: {
                to: 'call',
    ...
    const sendRetryRequest = async (node, forceIncludeKeys = false) => {
        ...
        if (retryRequestActiveJids.has(retryDedupeJid)) {
            logger.debug({ fromJid: retryDedupeJid, msgId }, 'Skipping duplicate retry request — already in-flight for this JID');
            return;
        }
            ...
            if (!attempt.proceed) {
                ...
                if (autoCleanCorrupted) {
                    ...
                    try {
                        ...
                        if (deletedCount > 0) {
                            logger.info({ msgId, jid: decryptionJid, targetedDevices: deletedCount }, `🔄 Session cleanup (retry exhausted) | Targeted: ${deletedCount} devices`);
                        }
                    }
                    catch (cleanupErr) {
        ...
        setTimeout(() => retryRequestActiveJids.delete(retryDedupeJid), 5000);
        const key = `${msgId}:${msgKey?.participant}`;
        ...
        if (!account)
            throw new Boom('Account not available', { statusCode: 401 });
    ...
    const handleGroupNotification = async (fullNode, child, msg) => {
        ...
        const affectedParticipant = affectedParticipantPn || (await resolveLidToPn(affectedParticipantLid, lidMapping, logger));
        // Store LID↔PN mappings from notification attributes
        const mappingsToStore = [];
        if (actingParticipantLid &&
            actingParticipantPn &&
        ...
        switch (child?.tag) {
            ...
            case 'not_ephemeral':
                ...
                break;
            case 'modify':
                const oldNumbers = await Promise.all(getBinaryNodeChildren(child, 'participant').map(async (p) => {
            ...
            case 'leave':
                ...
                const participants = await Promise.all(getBinaryNodeChildren(child, 'participant').map(async ({ attrs }) => {
    ...
    const sendMessagesAgain = async (key, ids, retryNode, receiptNode) => {
         ...
         */
        const verifyCanonicalDelete = async (reason) => {
            try {
                const lookupKeys = [sessionId];
                if (!isLidUser(participant)) {
                    const lid = await signalRepository.lidMapping.getLIDForPN(participant);
                    if (lid) {
    ...
    const handleMessage = async (node) => {
        ...
        try {
            ...
            const { fullMessage: msg, category, author, decrypt } = decryptMessageNode(node, authState.creds.me.id, authState.creds.me.lid || '', signalRepository, logger, msmsgSecretCache);
            ...
            await messageMutex.mutex(mutexKey, async () => {
                ...
                if (msg.messageStubType === proto.WebMessageInfo.StubType.CIPHERTEXT && msg.category !== 'peer') {
                    ...
                    const isOldCounterErr = stubError.includes('old counter') || stubError.includes('Over 2000 messages into the future');
                    if (isOldCounterErr) {
                        const encChild = getBinaryNodeChild(node, 'enc');
                        if (encChild?.attrs?.type === 'skmsg') {
                            logger.warn({ msgId: msg.key?.id, remoteJid: msg.key?.remoteJid, error: stubError }, 'skmsg sender-key stale: NACK 496 to stop retry loop (waiting for fresh SKDM)');
                    ...
                    if (msg.messageStubParameters?.[0] === NO_MESSAGE_FOUND_ERROR_TEXT) {
                        ...
                        if (messageType === 'bot_unavailable_fanout' ||
                            ...
                            logger.debug({ msgId: msg.key?.id, messageType }, 'CTWA: Skipping placeholder resend for unavailable fanout type');
                        ...
                        if (messageAge > PLACEHOLDER_MAX_AGE_SECONDS) {
                            logger.debug({ msgId: msg.key?.id, messageAge, maxAge: PLACEHOLDER_MAX_AGE_SECONDS }, 'CTWA: Skipping placeholder resend for old message');
                        ...
                        if (enableCTWARecovery && msg.key) {
                            ...
                            logger.info({ msgId, remoteJid: msgKey.remoteJid, messageAge }, 'CTWA: Message absent from node detected, scheduling placeholder resend from phone');
                            ...
                            logger.debug({ msgId: msg.key?.id, enableCTWARecovery }, 'CTWA recovery disabled or missing key, skipping placeholder resend');
                        }
                        await sendMessageAck(node);
                        acked = true;
                        return;
        ...
        if (pn.length === 13) {
            ...
            if (['2', '3', '4', '5'].includes(firstDigitAfterDDD)) {
                ...
                if (pn.endsWith('0')) {
                    ...
                    logger.debug({ original: pn, sanitized, firstDigit: firstDigitAfterDDD, type: 'landline' }, 'Call: Sanitized Brazilian landline number (removed trailing zero)');
                    return sanitized;
                }
            }
            ...
            if (['6', '7', '8', '9'].includes(firstDigitAfterDDD)) {
                logger.trace({ pn, firstDigit: firstDigitAfterDDD, type: 'mobile' }, 'Call: Valid Brazilian mobile number (13 digits correct, not sanitizing)');
                return pn;
            }
        }
        return pn;
    ...
    const handleCallInner = async (node) => {
        const { attrs } = node;
        const children = getAllBinaryNodeChildren(node);
        if (!children.length) {
        ...
        for (const infoChild of children) {
            ...
            if (resolvedChatId)
                call.chatId = resolvedChatId;
            if (resolvedFrom)
                call.from = resolvedFrom;
            if (resolvedLinkCreator)
                call.linkCreator = resolvedLinkCreator;
        ...
        if (attrs.error) {
            ...
            if (is463) {
                ...
                if (msgId && jid && !tcTokenRetriedMsgIds.has(retryKey)) {
                    ...
                    setTimeout(() => tcTokenRetriedMsgIds.delete(retryKey), 60000);
                    (async () => {
                        try {
                            await delay(1500);
                            const msg = (await getMessage(key)) ??
    ...
    const offlineNodeProcessor = makeOfflineNodeProcessor(nodeProcessorMap, {
        isWsOpen: () => ws.isOpen,
        onUnexpectedError,
        yieldToEventLoop
    }, 25);
    ...
    registerSocketEndHandler(() => {
        ...
        for (const t of pdoCleanupTimers)
            clearTimeout(t);
