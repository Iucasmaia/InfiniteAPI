...
import { generateMessageIDV2, resolveLidToPn, unixTimestampSeconds } from '../Utils/index.js';
...
export const makeGroupsSocket = (config) => {
    ...
    const normalizeGroupMetadata = async (metadata) => {
        ...
        if (resolvedSubjectOwner)
            metadata.subjectOwner = resolvedSubjectOwner;
        return metadata;
    };
