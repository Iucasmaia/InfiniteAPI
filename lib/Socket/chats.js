...
import { DEFAULT_CACHE_MAX_KEYS, DEFAULT_CACHE_TTLS, HISTORY_SYNC_PAUSED_TIMEOUT_MS, PROCESSABLE_HISTORY_TYPES } from '../Defaults/index.js';
import { ALL_WA_PATCH_NAMES } from '../Types/index.js';
import { SyncState } from '../Types/State.js';
import { chatModificationToAppPatch, decodePatches, decodeSyncdSnapshot, encodeSyncdPatch, ensureLTHashStateVersion, extractSyncdPatches, generateProfilePicture, getHistoryMsg, isAppStateSyncIrrecoverable, isMissingKeyError, MAX_SYNC_ATTEMPTS, newLTHashState, processSyncAction, resolveLidToPn } from '../Utils/index.js';
import { makeKeyedMutex, makeMutex } from '../Utils/make-mutex.js';
import processMessage from '../Utils/process-message.js';
import { buildTcTokenFromJid, buildTcTokenNode } from '../Utils/tc-token-utils.js';
import { getBinaryNodeChild, getBinaryNodeChildren, isAnyLidUser, isAnyPnUser, jidDecode, jidNormalizedUser, reduceBinaryNodeToDictionary, S_WHATSAPP_NET } from '../WABinary/index.js';
import { USyncQuery, USyncUser } from '../WAUSync/index.js';
import { makeSocket } from './socket.js';
export const makeChatsSocket = (config) => {
    const { logger, markOnlineOnConnect, fireInitQueries, appStateMacVerification, shouldIgnoreJid, shouldSyncHistoryMessage, getMessage } = config;
    const sock = makeSocket(config);
    const { ev, ws, authState, generateMessageTag, sendNode, query, signalRepository, onUnexpectedError, sendUnifiedSession, skipOfflineBuffer: socketSkippedOfflineBuffer, registerSocketEndHandler } = sock;
     ...
     */
    const appStateSyncKeyCache = new LRUCache({
    ...
    const newAppStateChunkHandler = (isInitialSync) => {
        return {
            onMutation(mutation) {
                ...
                if (!me)
                    throw new Boom('Not authenticated', { statusCode: 401 });
                processSyncAction(mutation, ev, me, isInitialSync ? { accountSettings: authState.creds.accountSettings } : undefined, logger);
            }
        };
    };
     ...
     *   </iq>
     */
    const profilePictureUrl = async (jid, type = 'preview', timeoutMs, opts) => {
        const normalizedJid = jidNormalizedUser(jid);
        ...
        const pictureAttrs = { type, query: 'url' };
        if (opts?.existingId)
            pictureAttrs.id = opts.existingId;
        if (opts?.invite)
            pictureAttrs.invite = opts.invite;
        if (opts?.personaId)
            pictureAttrs.persona_id = opts.personaId;
        if (opts?.commonGid)
            pictureAttrs.common_gid = opts.commonGid;
        const pictureNode = { tag: 'picture', attrs: pictureAttrs };
    ...
    const handlePresenceUpdate = async ({ tag, attrs, content }) => {
        let presence;
    ...
    ev.on('connection.update', ({ connection, receivedPendingNotifications }) => {
        ...
        if (isReconnection) {
            logger.info({ accountSyncCounter: authState.creds.accountSyncCounter, socketSkippedOfflineBuffer }, 'Reconnection detected, skipping AwaitingInitialSync wait. Transitioning to Online immediately.');
            blockedCollections.clear();
            syncState = SyncState.Online;
            const accountSyncCounter = (authState.creds.accountSyncCounter || 0) + 1;
            ev.emit('creds.update', { accountSyncCounter });
            ...
            resyncAppState(ALL_WA_PATCH_NAMES, true).catch(err => logger.warn({ err }, 'Background app state resync failed (non-critical on reconnection)'));
    ...
    ev.on('lid-mapping.update', async (mappings) => {
        try {
            ...
            logger.debug({ count: mappings.length, stored: result.stored, skipped: result.skipped, errors: result.errors }, 'stored LID-PN mappings from update event');
            if (result.stored > 0) {
                logger.info({ count: mappings.length, stored: result.stored }, 'fallback LID mappings are now available from update event');
            }
            ...
            if (result.errors > 0) {
                logger.warn({ errors: result.errors, total: mappings.length, notified: mergeNotifications.length }, 'some LID-PN mappings failed to store, but merge notifications were sent');
            }
        }
        catch (error) {
