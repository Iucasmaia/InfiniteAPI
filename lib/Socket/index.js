 ...
 */
const createCacheLogger = (logger) => {
    if (!logger)
        return undefined;
    return {
 ...
 */
const versionsAreDifferent = (v1, v2) => {
    return v1[0] !== v2[0] || v1[1] !== v2[1] || v1[2] !== v2[2];
};
/**
 * Checks if a version change is critical (major or minor version changed)
 */
const isCriticalVersionChange = (oldVersion, newVersion) => {
 ...
 * ```
 */
export const makeWASocketAutoVersion = async (config) => {
    const mergedConfig = {
        ...DEFAULT_CONNECTION_CONFIG,
        ...config
    };
    ...
    let trackedVersion = [...mergedConfig.version];
