...
import { DEF_CALLBACK_PREFIX, DEF_TAG_PREFIX, DEFAULT_SESSION_CLEANUP_CONFIG, INITIAL_PREKEY_COUNT, MIN_PREKEY_COUNT, MIN_UPLOAD_INTERVAL, NOISE_WA_HEADER, UPLOAD_TIMEOUT } from '../Defaults/index.js';
import { makeSessionActivityTracker } from '../Signal/session-activity-tracker.js';
import { makeSessionCleanup } from '../Signal/session-cleanup.js';
...
import { getPlatformId, isAndroidBrowser } from '../Utils/browser-utils.js';
import { decrementActiveConnections, incrementActiveConnections, recordConnectionAttempt, recordConnectionError } from '../Utils/prometheus-metrics.js';
import { createUnifiedSessionManager, extractServerTime, shouldEnableUnifiedSession } from '../Utils/unified-session.js';
...
import { createOfflineBufferState } from './offline-buffer-state.js';
/**
...
export const makeSocket = (config) => {
    ...
    enableUnifiedSession: enableUnifiedSessionConfig } = config;
    // Resolve enableUnifiedSession: explicit config > env var > default (true)
    const enableUnifiedSession = enableUnifiedSessionConfig !== undefined ? enableUnifiedSessionConfig : shouldEnableUnifiedSession();
    // Unified Session Manager will be initialized after sendNode is defined
    let unifiedSessionManager;
    ...
    const sendUnifiedSession = async (trigger = 'manual') => {
    ...
    const query = async (node, timeoutMs) => {
        ...
        const responsePromise = waitForMessage(msgId, timeoutMs);
    ...
    const sessionCleanup = makeSessionCleanup(keys, signalRepository.lidMapping, sessionActivityTracker, logger, sessionCleanupConfig);
    let lastDateRecv;
    let epoch = 1;
    let keepAliveReq;
    let qrTimer;
    ...
    const end = async (error) => {
        ...
        if (isSessionError) {
            logger.warn({ statusCode, reason: DisconnectReason[statusCode] }, '🔴 Socket-level session error - consumer should recreate socket');
        }
    ...
    ws.on('CB:success', async (node) => {
        ...
        const _cleanupStartTimer = setTimeout(() => sessionCleanup.start(), 5000);
    ...
    const OFFLINE_BUFFER_TIMEOUT_MS = 2000;
    // State machine lives in `./offline-buffer-state` so it can be unit
    // tested directly without spinning up a socket. (audit TST-06)
    const offlineBuffer = createOfflineBufferState(() => ev.flush(), () => logger.warn({ timeoutMs: OFFLINE_BUFFER_TIMEOUT_MS }, 'perf: offline-buffer safety timeout reached, force-flushing before CB:ib,,offline'), OFFLINE_BUFFER_TIMEOUT_MS);
    process.nextTick(() => {
        if (creds.me?.id && !skipOfflineBuffer) {
            ...
            logger.info('perf: skipping offline-phase buffer — reconnect of existing session, messages will be delivered immediately');
        }
     ...
     */
    const fetchAccountReachoutTimelock = async (emitUpdate = true) => {
        // 10s timeout — fire-and-forget chamadores não podem ficar com
        // promise pendente indefinida em caso de socket instável (audit
        // ROBUST-01).
        const queryResult = await promiseTimeout(10000, (resolve, reject) => executeWMexQuery({}, QueryIds.REACHOUT_TIMELOCK, XWAPaths.xwa2_fetch_account_reachout_timelock, query, generateMessageTag)
            .then(resolve)
            .catch(reject));
