...
export const prepareStickerPackMessage = async (stickerPack, options) => {
    ...
    const processedStickers = await Promise.all(stickers.map(async (sticker, i) => {
        if (!sticker)
            return null; // Skip undefined stickers
                    ...
                    try {
                        ...
                        if (compressed70.length <= MAX_STICKER_SIZE) {
                            ...
                            if (compressed50.length <= MAX_STICKER_SIZE) {
                                ...
                                // Still too large
                                throw new Boom(`Sticker ${i + 1} still exceeds 1MB after compression (${(compressed50.length / 1024).toFixed(2)}KB). ` +
                                    `Please use a smaller image.`, { statusCode: 400 });
                            }
                        }
                    }
                    catch (compressionError) {
                        // If compression fails, throw error about size
                        throw new Boom(`Sticker ${i + 1} exceeds 1MB and compression failed: ${compressionError.message}`, { statusCode: 400 });
                    }
                }
                else {
                    // No Sharp available, can't compress
                    throw new Boom(`Sticker ${i + 1} exceeds the 1MB hard limit (${(webpBuffer.length / 1024).toFixed(2)}KB). ` +
