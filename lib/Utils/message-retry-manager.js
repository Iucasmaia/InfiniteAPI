...
export class MessageRetryManager {
    ...
    shouldRecreateSession(jid, hasSession, errorCode) {
        ...
        if (errorCode !== undefined && MAC_ERROR_CODES.has(errorCode)) {
            ...
            const MAC_ERROR_COOLDOWN_MS = 1000; // 1 second — WABA recovers faster
            if (prevTime && now - prevTime < MAC_ERROR_COOLDOWN_MS) {
                const reasonName = RetryReason[errorCode] || `code_${errorCode}`;
                this.logger.debug({ jid, errorCode: reasonName, msSinceLast: now - prevTime }, 'MAC error session recreation skipped — cooldown active');
                return {
                    reason: '',
                    recreate: false
                };
