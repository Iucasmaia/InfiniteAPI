...
import { DEFAULT_CACHE_MAX_KEYS, DEFAULT_CACHE_TTLS } from '../Defaults/index.js';
import { isJidMetaAI, jidDecode, jidNormalizedUser } from '../WABinary/jid-utils.js';
import { isNodeCacheFullError } from './cache-utils.js';
import { aesDecryptGCM, hkdf } from './crypto.js';
import { compactError } from './error-log-utils.js';
 ...
 */
const KEY_LENGTH = 32;
export const makeMsmsgSecretCache = () => new NodeCache({
 ...
 */
export const extractMsmsgStanzaInfo = (stanza) => {
    ...
    for (const child of stanza.content) {
        ...
        switch (child.tag) {
            ...
            case 'meta':
                if (attrs?.target_id)
                    info.targetId = attrs.target_id;
                if (attrs?.target_sender_jid)
                    info.targetSenderJid = attrs.target_sender_jid;
                break;
            case 'bot':
                if (attrs && 'edit_target_id' in attrs)
                    info.botEditTargetId = attrs.edit_target_id;
                if (attrs?.edit)
                    info.botEditType = attrs.edit;
                break;
        }
    }
    if (!hasMsmsgEnc || !info.targetId)
        return null;
    return info;
};
/**
 ...
 */
export const cacheMessageSecretIfPresent = (cache, msg, msgKey, logger) => {
    if (!cache)
        return;
    const secret = msg.messageContextInfo?.messageSecret;
    if (!secret || !msgKey.remoteJid || !msgKey.id)
        return;
    ...
    try {
        ...
        if (!isNodeCacheFullError(err))
            throw err;
        logger?.debug({ cacheKey, msmsgCacheSize: cache.keys().length }, 'msmsg secret cache full (ECACHEFULL), dropping write');
    }
};
/**
...
const INNER_OR_LAST = new Set(['inner', 'last']);
/**
 ...
 */
export const decryptMsmsgBotMessage = (input) => {
    const { ciphertext, stanzaInfo, stanzaId, authorJid, chatJid, isGroup, isFbidBot, meLid, meId, cache, logger, lidToPn } = input;
    ...
    if (!messageSecret)
        throw new OrphanMsmsgError(cacheKey);
    ...
    const primaryHkdfStanzaId = useEditTargetForHkdf ? stanzaInfo.botEditTargetId : stanzaId;
    ...
    try {
        ...
        logger?.trace({ cacheKey, hkdfStanzaId: primaryHkdfStanzaId, isFbidBot, botEditType: stanzaInfo.botEditType }, 'msmsg decrypted');
        ...
        if (!isFbidBot && stanzaInfo.botEditTargetId && stanzaInfo.botEditTargetId !== primaryHkdfStanzaId) {
            try {
                ...
                logger?.debug({ cacheKey, fallbackHkdfStanzaId: stanzaInfo.botEditTargetId }, 'msmsg decrypted after botEditTargetId fallback');
                return decodeDecryptedMsmsg(plaintext);
            }
            catch (fallbackErr) {
                logger?.warn({
        ...
        logger?.warn({ cacheKey, isFbidBot, botEditType: stanzaInfo.botEditType, err: compactError(firstErr) }, 'msmsg decryption failed');
        throw firstErr;
    }
};
 ...
 */
const decodeDecryptedMsmsg = (plaintext) => {
    try {
 ...
 */
export const isMsmsgBotConversation = (chatOrAuthorJid) => {
    if (!chatOrAuthorJid)
        return false;
    return !!isJidMetaAI(chatOrAuthorJid);
};
//# sourceMappingURL=meta-ai-msmsg.js.map