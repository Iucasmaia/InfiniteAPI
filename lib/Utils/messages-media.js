...
export const downloadEncryptedContent = async (downloadUrl, { cipherKey, iv, macKey }, { startByte, endByte, options } = {}) => {
    let bytesFetched = 0;
    let startChunk = 0;
            ...
            try {
                if (verifyMac && hmac) {
                    ...
                    if (trailerBuf.length !== HMAC_TRAILER_LEN || !Crypto.timingSafeEqual(expected, trailerBuf)) {
                        return callback(new Boom('media HMAC verification failed', {
                            statusCode: 401,
                ...
                if (!aes) {
                    return callback(new Boom('media stream too short — no ciphertext bytes received', {
                        statusCode: 400
                    }));
                }
...
export const getWAUploadToServer = ({ customUploadHosts, fetchAgent, logger, options }, refreshMediaConn) => {
    return async (filePath, { mediaType, fileEncSha256B64, timeoutMs, newsletter }) => {
        ...
        if (pathSegment === undefined) {
            throw new Error(`No upload path configured for mediaType=${mediaType}` +
