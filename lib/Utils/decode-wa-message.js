...
import { cacheMessageSecretIfPresent, decryptMsmsgBotMessage, extractMsmsgStanzaInfo, OrphanMsmsgError } from './meta-ai-msmsg.js';
import { retry, RetryExhaustedError } from './retry-utils.js';
/**
 ...
 */
export const unwrapDeviceSentMessage = (msg) => {
 ...
 */
export const DECRYPTION_RETRY_OPTIONS = {
        ...
        async decrypt() {
            ...
            if (Array.isArray(stanza.content)) {
                for (const { tag, attrs, content } of stanza.content) {
                    ...
                    let msgBuffer = EMPTY_UINT8_ARRAY;
                    ...
                    try {
                        ...
                        switch (e2eType) {
                            case 'skmsg':
                                msgBuffer = await retry(() => repository.decryptGroupMessage({
                                    group: sender,
                                    authorJid: author,
                                    msg: content
                                }), {
                                    ...
                                    onRetry: (error, attempt, delay) => {
                                        logger.debug({ error: error.message, attempt, delay, group: sender, author }, 'Retrying group message decryption');
                                    }
                                });
                                break;
                            case 'pkmsg':
                            case 'msg':
                                msgBuffer = await retry(() => repository.decryptMessage({
                                    jid: decryptionJid,
                                    type: e2eType,
                                    ciphertext: content
                                }), {
                                    ...DECRYPTION_RETRY_OPTIONS,
                                    onRetry: (error, attempt, delay) => {
                                        logger.debug({ error: error.message, attempt, delay, jid: decryptionJid, type: e2eType }, 'Retrying message decryption');
                                    }
                                });
                                break;
                            case 'plaintext':
                        ...
                        if (isCorrupted) {
                            ...
                            logger.error({ ...compactContext, err: originalError }, isRetryExhausted
 ...
 */
export function isCorruptedSessionError(error) {
    const errorMessage = error?.message || error?.toString() || '';
 ...
 */
export async function cleanupCorruptedSession(jid, repository, logger) {
