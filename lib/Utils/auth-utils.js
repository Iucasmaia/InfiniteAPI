...
import { safeCacheSet as sharedSafeCacheSet } from './cache-utils.js';
...
const heldLocksAcquire = (heldLocks, lockKey) => {
...
const heldLocksRelease = (heldLocks, lockKey) => {
            ...
            if (idsToFetch.length) {
                ...
                for (const id of idsToFetch) {
                    ...
                    if (item !== null && item !== undefined) {
                        ...
                        await safeCacheSet(getUniqueId(type, id), item);
                    }
                }
            }
            return data;
...
export const addTransactionCapability = (state, logger, { maxCommitRetries, delayBetweenTriesMs }) => {
    ...
    async function commitWithRetry(mutations) {
        ...
        const lockRefs = Object.keys(mutations).map(type => ({
            namespace: '__type__',
            id: type
        }));
    ...
    return {
        ...
        set: async (data) => {
            ...
            if (!ctx) {
                ...
                await Promise.all(types.map(type => locks.withLock({ namespace: '__type__', id: type }, async () => {
                    if (type === 'pre-key') {
            ...
            return locks.withLocks(lockRefs, async () => {
                ...
                if (existing) {
                    ...
                    for (const k of newLockKeys)
                        heldLocksAcquire(existing.heldLocks, k);
                    try {
                        return await work();
                    }
                    finally {
                        for (const k of newLockKeys)
                            heldLocksRelease(existing.heldLocks, k);
                        activeTransactions--;
                    }
                }
                const ctx = {
                    ...
                    mutations: {},
                    dbQueries: 0,
                    heldLocks: new Map(newLockKeys.map(k => [k, 1])),
                    sealed: false
                };
        ...
        destroy: async () => {
            ...
            const MAX_DRAIN_WAIT_MS = Math.max(5000, maxCommitRetries * delayBetweenTriesMs + 2000);
            ...
            while (activeTransactions > 0) {
                if (Date.now() - startedAt >= MAX_DRAIN_WAIT_MS) {
                    logger.warn({ activeTransactions, waitedMs: MAX_DRAIN_WAIT_MS }, 'destroy: drain wait timed out — proceeding with teardown');
                    break;
                }
                const now = Date.now();
