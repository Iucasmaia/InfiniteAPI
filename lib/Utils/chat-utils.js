...
import { expandAppStateKeys } from './wasm-bridge.js';
...
export const decodeSyncdSnapshot = async (name, snapshot, getAppStateSyncKey, minimumVersionNumber, validateMacs = true) => {
    ...
    const { hash, indexValueMap } = await decodeSyncdMutations(snapshotRecords, newState, getAppStateSyncKey, areMutationsRequired
        ? mutation => {
