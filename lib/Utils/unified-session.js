...
export class UnifiedSessionManager {
    ...
    async send(trigger = 'manual') {
        ...
        if (trigger !== 'login' && !this.canSend()) {
            this.options.logger.trace?.({ trigger, lastSent: this.state.lastSentTime }, 'Unified session rate limited, skipping');
            return;
        }
        ...
        const sendOperation = async () => {
            ...
            this.options.logger.debug?.({ sessionId, trigger, latency, sendCount: this.state.sendCount }, 'Unified session telemetry sent');
            // Record metrics
...
UnifiedSessionManager.MIN_SEND_INTERVAL_MS = TimeMs.Minute;
// ============================================
// Factory Function
// ============================================
/**
 ...
 * ```
 */
export function createUnifiedSessionManager(options = {}) {
 ...
 */
export function extractServerTime(node) {
    const timeAttr = node.attrs?.t;
    if (timeAttr === undefined || timeAttr === null) {
        return undefined;
    }
    const parsed = typeof timeAttr === 'string' ? parseInt(timeAttr, 10) : typeof timeAttr === 'number' ? timeAttr : undefined;
 ...
 */
export function shouldEnableUnifiedSession() {
