...
import { logEventBuffer } from './baileys-logger.js';
...
const parseFloatEnv = (raw, fallback) => {
    if (raw === undefined || raw === '')
        return fallback;
...
class AdaptiveTimeoutCalculator {
    constructor(minTimeout, maxTimeout, initialTimeout) {
        this.eventTimestamps = [];
        this.windowSize = 100; // Track last 100 events
 ...
 */
export const makeEventBuffer = (logger, configOverride) => {
    // Load configuration
    const config = {
        ...loadBufferConfig(),
        ...configOverride
     ...
     */
    const pendingFunctionTimeouts = new Set();
    let bufferCount = 0;
    ...
    const adaptiveTimeout = new AdaptiveTimeoutCalculator(config.minBufferTimeoutMs, config.maxBufferTimeoutMs, config.bufferTimeoutMs);
    ...
    const recordMetrics = (eventType, count) => {
        if (metricsModule) {
    ...
    const recordFlushMetrics = (eventCount, forced, cacheSize) => {
        if (metricsModule) {
    ...
    const clearAllTimers = () => {
        ...
        for (const t of pendingFunctionTimeouts)
            clearTimeout(t);
        ...
        case 'contacts.upsert':
            for (const contact of eventData) {
                ...
                if (existingContactUpdate) {
                    upsert = Object.assign(existingContactUpdate, trimUndefined(contact));
