...
import { areJidsSameUser, isAnyLidUser, isAnyPnUser, isHostedLidUser, isHostedPnUser, isJidBroadcast, isJidStatusBroadcast, isLidUser, jidDecode, jidEncode, jidNormalizedUser } from '../WABinary/index.js';
 ...
 */
export const resolveLidToPn = async (jid, lidMapping, logger) => {
    if (!jid) {
        return undefined;
    }
    if (isAnyLidUser(jid)) {
 ...
 */
export const normalizeKeyLidToPn = async (key, lidMapping, logger) => {
...
export const normalizeMessageJids = async (message, signalRepository, logger) => {
...
export const isRealMessage = (message) => {
    ...
    return ((!!normalizedContent || REAL_MSG_STUB_TYPES.has(stubType) || REAL_MSG_REQ_ME_STUB_TYPES.has(stubType)) &&
        hasSomeContent &&
...
const processMessage = async (message, { shouldProcessHistoryMsg, placeholderResendCache, ev, creds, signalRepository, keyStore, logger, options, getMessage }) => {
    ...
    if (protocolMsg) {
        ...
        const isFromOwnAccount = message.key.fromMe ||
                ...
                if (process) {
                    ...
                    if (data.lidPnMappings?.length) {
                        ...
                        try {
                            ...
                            logger?.debug({ stored: result.stored, skipped: result.skipped, errors: result.errors }, 'stored LID-PN mappings from history sync');
                            // eslint-disable-next-line max-depth
                            if (result.stored > 0) {
            ...
            case proto.Message.ProtocolMessage.Type.PEER_DATA_OPERATION_REQUEST_RESPONSE_MESSAGE:
                ...
                if (response) {
                    ...
                    const cachedData = response.stanzaId
                        ? await placeholderResendCache?.get(response.stanzaId)
                        : undefined;
                        ...
                        if (retryResponse) {
                            ...
                            if (cachedData && typeof cachedData === 'object') {
                                ...
                                if (cachedData.participantAlt && webMessageInfo.key) {
                                    const msgKey = webMessageInfo.key;
                                    // eslint-disable-next-line max-depth
                                    if (!msgKey.participantAlt) {
                                        msgKey.participantAlt = cachedData.participantAlt;
                                        logger?.debug({ msgId: webMessageInfo.key?.id, participantAlt: cachedData.participantAlt }, 'CTWA: Restored participantAlt (LID) from cached metadata');
                                    }
                                }
                            ...
                            if (webMessageInfo.key && signalRepository) {
                                await normalizeKeyLidToPn(webMessageInfo.key, signalRepository.lidMapping, logger);
                            }
                    ...
                    if (recoveredCount > 0) {
                        ...
                        logger?.debug({ recoveredCount, requestId: response.stanzaId }, 'CTWA: Placeholder resend response processed');
                    }
                }
                break;
