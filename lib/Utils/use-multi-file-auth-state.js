...
export const useMultiFileAuthState = async (folder) => {
     ...
     */
    const tryReadAndParse = async (path) => {
        let raw;
        try {
    ...
    // succeeds atomically).
    const renameOverwrite = async (from, to) => {
        try {
    ...
    const writeData = async (data, file) => {
        ...
        try {
            ...
            await renameOverwrite(filePath, bakPath).catch((err) => {
     ...
     */
    const unlinkIgnoreMissing = async (path) => {
        try {
            await unlink(path);
        }
        catch (err) {
     ...
     * that intact.
     */
    function decodeIdForType(type, encodedId) {
        if (type === 'sender-key') {
    ...
    async function* iterateType(type) {
        ...
        const collidingPrefix = type === 'sender-key' ? `${fixFileName('sender-key-memory')}-` : null;
        const seenEncodedIds = new Set();
        // Pass 1 — primary `*.json` files.
        for (const filename of entries) {
            if (!filename.startsWith(prefix) || !filename.endsWith('.json'))
                continue;
            if (collidingPrefix && filename.startsWith(collidingPrefix))
                continue;
        ...
        for (const filename of entries) {
            if (!filename.startsWith(prefix) || !filename.endsWith(BAK_SUFFIX))
                continue;
            if (collidingPrefix && filename.startsWith(collidingPrefix))
                continue;
            const encodedId = filename.slice(prefix.length, -BAK_SUFFIX.length);
            if (seenEncodedIds.has(encodedId))
                continue;
            seenEncodedIds.add(encodedId);
            ...
            keys: {
                ...
                clear: async () => {
                    ...
                    await Promise.all(entries
                        .filter(f => f !== 'creds.json' && (f.endsWith('.json') || f.endsWith('.json.tmp') || f.endsWith('.json.bak')))
                        .map(f => unlinkIgnoreMissing(join(folder, f))));
                },
                list: async function* (type) {
                    for await (const entry of iterateType(type)) {
                        // eslint-disable-next-line @typescript-eslint/no-explicit-any
                        let value = await readData(entry.filename);
                        if (type === 'app-state-sync-key' && value) {
                            value = proto.Message.AppStateSyncKeyData.fromObject(value);
                        }
                        if (value !== null && value !== undefined) {
