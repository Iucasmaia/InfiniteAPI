...
import { downloadContentFromMessage, encryptedStream, extractImageThumb, generateThumbnail, getAudioDuration, getAudioWaveform, getRawMediaUploadData, getStream } from './messages-media.js';
...
export const prepareWAMessageMedia = async (message, options) => {
    ...
    for (const key of MEDIA_KEYS) {
        ...
        if (key in message && typeof message[key] !== 'boolean') {
            mediaType = key;
        }
    }
    if (!mediaType) {
    ...
    const isValidWaveform = (w) => {
        if (!(w instanceof Uint8Array) || w.length !== 64)
            return false;
        for (let i = 0; i < 64; i++) {
            const v = w[i];
            if (v > 100)
    ...
    const [{ mediaUrl, directPath }] = await Promise.all([
        ...
        (async () => {
            try {
                if ((requiresThumbnailComputation || requiresDurationComputation || requiresWaveformProcessing) &&
                    !originalFilePath) {
 ...
 */
export const formatNativeFlowButton = (button) => {
 ...
 * ```
 */
export const generateButtonMessage = async (options, mediaOptions) => {
 ...
 * ```
 */
export const generateCarouselMessage = async (options, mediaOptions) => {
    ...
    if (cardsWithoutMedia.length > 0) {
        mediaOptions?.logger?.warn({ cardsWithoutMedia: cardsWithoutMedia.length, totalCards: cards.length }, '[CAROUSEL] WARNING: Cards without media will NOT render on WhatsApp Web. Every card MUST have an image or video.');
    }
    const recoverCarouselImageMetadata = async (imageMessage, cardImage, cardTitle, uploadOptions) => {
        if (!imageMessage) {
            return;
        }
        ...
        if (hasMedia && mediaOptions) {
            if (card.image) {
                ...
                if (imageMessage && !imageMessage.jpegThumbnail) {
                    mediaOptions.logger?.warn({ cardTitle: card.title }, '[CAROUSEL] imageMessage missing jpegThumbnail - Web may not render');
                }
                if (imageMessage && (!imageMessage.height || !imageMessage.width)) {
                    mediaOptions.logger?.warn({ cardTitle: card.title, height: imageMessage.height, width: imageMessage.width }, '[CAROUSEL] imageMessage missing dimensions - setting defaults');
 ...
 */
const validateListSections = (sections, buttonText) => {
    ...
    for (const section of sections) {
        if (section.title && section.title.length > LIST_LIMITS.MAX_SECTION_TITLE) {
            throw new Boom(`Section title max ${LIST_LIMITS.MAX_SECTION_TITLE} characters, got ${section.title.length}: "${section.title}"`, { statusCode: 400 });
        }
        ...
        if (section.rows.length > LIST_LIMITS.MAX_ROWS_PER_SECTION) {
            throw new Boom(`Maximum ${LIST_LIMITS.MAX_ROWS_PER_SECTION} rows per section, got ${section.rows.length} in "${section.title}"`, { statusCode: 400 });
        }
        for (const row of section.rows) {
            ...
            if (row.title && row.title.length > LIST_LIMITS.MAX_ROW_TITLE) {
                throw new Boom(`Row title max ${LIST_LIMITS.MAX_ROW_TITLE} characters, got ${row.title.length}: "${row.title}"`, { statusCode: 400 });
            }
            if (row.description && row.description.length > LIST_LIMITS.MAX_ROW_DESCRIPTION) {
                throw new Boom(`Row description max ${LIST_LIMITS.MAX_ROW_DESCRIPTION} characters, got ${row.description.length}: "${row.description.substring(0, 30)}..."`, { statusCode: 400 });
            }
        }
...
export const generateListMessage = (options) => {
 ...
 * ```
 */
export const generateProductListMessage = (options) => {
 ...
 * ```
 */
export const generateProductCarouselMessage = (options) => {
 ...
 */
export const generateButtonMessageLegacy = (buttons, text, footer) => {
 ...
 */
export const generateListMessageLegacy = (listInfo, title, description, buttonText, footer) => {
...
export const generateWAMessageContent = async (message, options) => {
    ...
    if (hasNonNullishProperty(message, 'nativeButtons')) {
        const nativeMsg = message;
        const buttons = nativeMsg.nativeButtons;
        if (!buttons || buttons.length === 0) {
        ...
        if (allQuickReply) {
            ...
            buttonsMessage.buttons = buttons.map((btn, idx) => ({
    ...
    // Check for nativeList
        ...
        const generated = generateListMessageLegacy({ sections: listMsg.nativeList.sections }, listMsg.title || '', listMsg.text || '', listMsg.nativeList.buttonText, listMsg.footer);
    ...
    // Check for productCarousel (WhatsApp Business catalog products)
        ...
        const productCarouselMsg = message;
    ...
    // Interactive messages: buttons, lists, templates
        ...
        const buttonsMessage = {
            ...
            headerType: message.headerType || proto.Message.ButtonsMessage.HeaderType.EMPTY
        };
        buttonsMessage.buttons = message.buttons.map((btn, idx) => ({
        ...
        const hydratedButtons = message.templateButtons.map((btn) => {
        ...
        validateListSections(message.sections, message.buttonText);
        const listMessage = {
            title: message.title,
            description: message.text,
            buttonText: message.buttonText || 'View options',
            footerText: message.footerText,
            listType: proto.Message.ListMessage.ListType.SINGLE_SELECT
        };
        listMessage.sections = message.sections.map((section) => ({
            title: section.title,
            rows: section.rows.map((row) => ({
        ...
        options.logger?.info({ expectedImageCount, expectedVideoCount, totalItems: medias.length }, 'Album message validated - sending will be handled by relayMessage');
    }
    ...
    else if ((hasNonNullishProperty(message, 'image') || hasNonNullishProperty(message, 'video')) &&
        hasNonNullishProperty(message, 'buttons')) {
        const mediaType = hasNonNullishProperty(message, 'image') ? 'image' : 'video';
        const headerType = mediaType === 'image'
...
export const generateWAMessageFromContent = (jid, message, options) => {
    ...
    if (quoted && !isJidNewsletter(jid)) {
        ...
        const innerContent = innerMessage[key];
        const contextInfo = (innerContent &&
            typeof innerContent === 'object' &&
            'contextInfo' in innerContent &&
            innerContent.contextInfo) ||
            {};
    ...
    const isCarouselEphemeral = !!message?.interactiveMessage?.carouselMessage ||
    ...
    const isCarouselMsg = !!message?.interactiveMessage?.carouselMessage ||
        !!message?.viewOnceMessage?.message?.interactiveMessage?.carouselMessage;
    if (!isCarouselMsg) {
...
export const downloadMediaMessage = async (message, type, options, ctx) => {
    const result = await downloadMsg().catch(async (error) => {
        ...
        const httpStatus = (typeof error?.output?.statusCode === 'number' ? error.output.statusCode : undefined) ??
