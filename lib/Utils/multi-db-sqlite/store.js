...
import { MULTI_DB_FILES, SCHEMAS } from './schemas/index.js';
/**
...
async function loadBetterSqlite3() {
    try {
        ...
        const e = new Error('`better-sqlite3` is required for `MultiDbSqliteStore`. Install it: `npm install better-sqlite3`.');
        e.cause = err;
        throw e;
    }
}
        ...
        try {
            for (const file of MULTI_DB_FILES) {
                ...
                if (fileMigrations && fileMigrations.length > 0) {
                    runMigrations(db, fileMigrations);
                }
                else {
