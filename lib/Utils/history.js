...
import { isAnyLidUser, isAnyPnUser, isJidBroadcast, isJidGroup, isJidNewsletter, jidDecode, jidNormalizedUser } from '../WABinary/index.js';
 ...
 * // Returns: undefined
 */
export function extractLidPnFromConversation(chatId, lidJid, pnJid) {
 ...
 * @returns LID-PN mapping if extractable, undefined otherwise
 */
export function extractLidPnFromMessage(remoteJid, remoteJidAlt, participant, participantAlt) {
        ...
        case proto.HistorySync.HistorySyncType.ON_DEMAND:
            for (const chat of (item.conversations ?? [])) {
                ...
                for (const item of msgs) {
                    ...
                    const messageMapping = extractLidPnFromMessage(message.key.remoteJid, message.key.remoteJidAlt, message.key.participant, message.key.participantAlt);
                    if (messageMapping) {
