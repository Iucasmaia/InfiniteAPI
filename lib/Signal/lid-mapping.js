...
import { isAnyLidUser, isAnyPnUser, isHostedPnUser, jidDecode, jidNormalizedUser, WAJIDDomains } from '../WABinary/index.js';
/**
 * Load configuration from environment variables
 * Includes bounds validation to prevent DoS from malicious values
 */
export function loadLIDMappingConfig() {
    // Helper to clamp values within safe bounds
    const clamp = (value, min, max) => Math.max(min, Math.min(max, isNaN(value) ? min : value));
    ...
    return {
        ...
        cacheTtlMs: clamp(cacheTtlMs, 60000, 30 * 24 * 60 * 60 * 1000),
        // Cache size: minimum 100, maximum 1,000,000
        maxCacheSize: clamp(maxCacheSize, 100, 1000000),
        ...
        retryDelayMs: clamp(retryDelayMs, 100, 60000),
...
export var LIDMappingErrorCode;
(function (LIDMappingErrorCode) {
    LIDMappingErrorCode["INVALID_JID"] = "INVALID_JID";
    LIDMappingErrorCode["INVALID_MAPPING"] = "INVALID_MAPPING";
    LIDMappingErrorCode["DATABASE_ERROR"] = "DATABASE_ERROR";
    LIDMappingErrorCode["USYNC_ERROR"] = "USYNC_ERROR";
    LIDMappingErrorCode["CACHE_ERROR"] = "CACHE_ERROR";
    LIDMappingErrorCode["DESTROYED"] = "DESTROYED";
})(LIDMappingErrorCode || (LIDMappingErrorCode = {}));
// ============================================================================
// MAIN CLASS
// ============================================================================
/**
...
export class LIDMappingStore {
    constructor(keys, logger, pnToLIDFunc, configOverride) {
    ...
    async getLIDsForPNs(pns) {
        ...
        return this.trackOperation(async () => {
            ...
            const failedPns = new Set();
            const pendingByPnUser = new Map();
            for (const pn of pns) {
                if (!isAnyPnUser(pn))
                    continue;
                const decoded = jidDecode(pn);
                ...
                for (const [pnUser, items] of pendingByPnUser.entries()) {
                    ...
                    for (const { pn, decoded } of items) {
                        if (lidUser && decoded) {
                            ...
                            const pnDevice = decoded.device ?? 0;
                            const deviceSpecificLid = this.buildDeviceSpecificJid(lidUserString, pnDevice, decoded.server === 'hosted' ? 'hosted.lid' : 'lid');
                            if (this.config.debugLogging) {
                                this.logger.trace({ pn, deviceSpecificLid, pnDevice }, 'getLIDForPN: mapping found');
                            }
                            successfulPairs[pn] = { lid: deviceSpecificLid, pn };
            ...
            if (failedPns.size > 0) {
                this.logger.warn({ failedCount: failedPns.size, totalRequested: pns.length }, 'Some PNs failed during getLIDsForPNs - results may be incomplete');
            }
    ...
    async getPNsForLIDs(lids) {
        ...
        return this.trackOperation(async () => {
            ...
            const pendingByLidUser = new Map();
            ...
            if (pendingByLidUser.size > 0) {
                ...
                const dbFailedReverseKeys = new Set();
            ...
            if (failedLids.size > 0) {
                this.logger.warn({ failedCount: failedLids.size, totalRequested: lids.length }, 'Some LIDs failed during getPNsForLIDs - results may be incomplete');
            }
     ...
     */
    async deleteMappingFromCache(pn) {
        this.checkDestroyed();
        // Track operation to prevent UAF during destroy()
        return this.trackOperation(async () => {
            if (!isAnyPnUser(pn))
    ...
    destroy() {
        ...
        if (this.operationsInProgress > 0) {
            this.logger.warn({ operationsInProgress: this.operationsInProgress }, '⚠️ Cannot destroy LIDMappingStore - operations still in progress. Resources will be cleaned by GC after completion.');
        ...
        for (let attempt = 1; attempt <= this.config.retryAttempts; attempt++) {
            try {
                ...
                if (attempt < this.config.retryAttempts) {
                    ...
                    this.logger.warn({ operationName, attempt, maxAttempts: this.config.retryAttempts, delay }, 'Operation failed, retrying');
                    await this.sleep(delay);
                }
            }
        }
        throw new LIDMappingError(`Operation ${operationName} failed after ${this.config.retryAttempts} attempts: ${lastError?.message}`, LIDMappingErrorCode.DATABASE_ERROR, { operationName, lastError: lastError?.message });
    }
    /**
     ...
     */
    async fetchFromUSync(usyncFetch, successfulPairs) {
        if (!this.pnToLIDFunc) {
     ...
     */
    async coalesceRequest(key, map, fetchFn) {
