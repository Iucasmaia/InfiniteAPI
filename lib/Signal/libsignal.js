...
import { createLIDMappingStoreWithSqlite } from '../Utils/multi-db-sqlite/factories.js';
import { metrics } from '../Utils/prometheus-metrics.js';
import { isAnyLidUser, isAnyPnUser, jidDecode, transferDevice, WAJIDDomains } from '../WABinary/index.js';
 ...
 */
function generateKeyFingerprint(key) {
 ...
 */
function extractIdentityFromPkmsg(ciphertext, logger) {
    ...
    try {
        ...
        logger?.debug({ error: error.message }, 'Failed to extract identity from pkmsg');
        return undefined;
    }
    finally {
        timer?.();
...
export function makeLibSignalRepository(auth, logger, pnToLIDFunc, options) {
    const { ev } = options || {};
    ...
    const keysCandidate = auth.keys;
    if (typeof keysCandidate.transactWith !== 'function') {
        throw new Error('makeLibSignalRepository: auth.keys is missing the `transactWith` method. ' +
    ...
    if (options?.multiDbStore) {
        lidMapping = createLIDMappingStoreWithSqlite({
            ...
            store: options.multiDbStore,
            logger,
            pnToLIDFunc
        });
    }
    else {
        lidMapping = new LIDMappingStore(auth.keys, logger, pnToLIDFunc);
    }
    // Identity key cache to avoid repeated storage reads
    const identityKeyCache = new LRUCache({
     ...
     */
    const pinResolutionForStorage = (rawAddr, wireJid) => {
        ...
        const pinned = Object.create(storage);
        pinned.loadSession = (id) => storage.loadSession(id === rawAddr ? wireJid : id);
        pinned.storeSession = (id, sess) => storage.storeSession(id === rawAddr ? wireJid : id, sess);
        pinned.saveIdentity = (id, identityKey) => storage.saveIdentity(id === rawAddr ? wireJid : id, identityKey);
        return pinned;
    };
    ...
    const deviceListCache = new LRUCache({
        max: 500,
    ...
    const repository = {
                    ...
                    if (identityKey) {
                        try {
                            ...
                            if (saveResult.changed) {
                                ...
                                logger.debug({ jid, addr: addrStr, fingerprint: saveResult.currentFingerprint }, 'New contact identity key saved (Trust On First Use)');
                            }
                        }
                        catch (error) {
                            // Log but don't fail decryption — identity tracking is best-effort.
                            logger.warn({ jid, error: error.message }, 'Failed to save identity key during decryption');
                        }
                    }
                }
                let result;
        ...
        async encryptMessage({ jid, data, useLegacyLock }) {
            ...
            if (useLegacyLock) {
                logger.info({ jid }, '[encryptMessage] WORKAROUND ACTIVE: running without inner tx wrap (interactive send, mirrors master H0 bypass)');
        ...
        async migrateSession(fromJid, toJid) {
            ...
            const migrationPromise = (async () => {
                ...
                let userDevices = deviceListCache.get(user);
                ...
                const deviceSessionKeyMap = new Map(); // sessionKey → originating JID
                const deviceSessionKeys = [];
                for (const device of uncachedDevices) {
                    const deviceNum = parseInt(device);
                    const jid = deviceNum === 99
                        ? `${user}:99@hosted`
                        : deviceNum === 0
                            ? `${user}@s.whatsapp.net`
                ...
                const migrationRecords = [
                    ...
                    ...migrationOps.flatMap(op => [
                        { type: 'session', id: op.fromAddr.toString() },
                        { type: 'session', id: op.toAddr.toString() }
                    ])
                ];
                return parsedKeys.transactWith({ records: migrationRecords }, async () => {
...
function signalStorage({ creds, keys }, lidMapping, identityKeyCache, ev, logger) {
    ...
    const pendingPreKeyDeletions = new Map();
    return {
        /**
        ...
        loadSession: async (id) => {
            try {
                ...
                logger?.warn({ id, err: e?.message }, 'loadSession failed, treating as missing');
                return null;
            }
            return null;
        },
            ...
            try {
                ...
                return await parsedKeys.transactWith({ records }, async () => {
                    ...
                    if (existingKey && !keysMatch) {
                        ...
                        const identityUpdates = { [wireJid]: identityKey };
                        if (wireJid !== id) {
